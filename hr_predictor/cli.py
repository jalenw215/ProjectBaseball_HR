...
def main() -> None:
    parser = argparse.ArgumentParser(description="MLB daily home run predictor")
    sub = parser.add_subparsers(dest="command", required=True)

    fetch_statcast = sub.add_parser("fetch-statcast", help="Download Baseball Savant Statcast CSV")
    ...
    training = sub.add_parser("build-training", help="Build leakage-safe hitter-game training rows")
    ...
    train = sub.add_parser("train", help="Train calibrated HR probability model")
    ...
    predict = sub.add_parser("predict-today", help="Generate daily HR rankings")
