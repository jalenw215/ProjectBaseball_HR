...
class PipelineRunner:
    ...
    def morning_refresh(self, prediction_date: str | None = None, model_path: Path | None = None) -> list[StepResult]:
        ...
        return self._run_sequence(
            [
                self.fetch_historical_data_tolerant,
    ...
    def fetch_historical_data_tolerant(self) -> StepResult:
        try:
            result = self.fetch_historical_data()
        except Exception as exc:
            result = StepResult(name="Fetch historical data", status="error", detail=str(exc))
        if result.status == "ok":
            return result

        cached_chunks = sorted(RAW_DIR.glob("statcast_????-??-??_????-??-??.csv"))
        if not self.config.statcast_file.exists() and not cached_chunks:
            return result

        self._log(f"WARN Fetch historical data failed ({self._classify_request_error(result.detail)}); using cached Statcast data: {result.detail}")
        self._emit("Fetch historical data unavailable; using cached Statcast data")
        if not self.config.statcast_file.exists():
            combine_statcast_chunks(cached_chunks, self.config.statcast_file)
        self._log(f"INFO Fetch historical data fallback succeeded: used cached Statcast data in {self.config.statcast_file}")
        return StepResult(
            name="Fetch historical data",
            status="ok",
            detail=f"used cached data in {self.config.statcast_file}",
        )
    ...
    def _fetch_historical_data(self) -> str:
        ...
        for start, end in month_chunks(self.config.start_date, self.config.end_date):
            ...
            if chunk_path.exists() and chunk_path.stat().st_size > 0:
                ...
                self._emit(f"Fetching Statcast {start} to {end}")
    ...
    def _predict_today(self, prediction_date: str, model_path: Path | None = None) -> str:
        ...
        try:
            schedule = fetch_schedule(prediction_date)
        except Exception as exc:
            self._log(f"WARN Schedule fetch failed ({self._classify_request_error(str(exc))}); skipping schedule cache: {exc}")
            self._emit("Schedule fetch unavailable; predictions were still written")
        else:
            ensure_parent(schedule_path).write_text(json.dumps(schedule, indent=2), encoding="utf-8")
        return f"wrote {output}"

    def _classify_request_error(self, message: str) -> str:
        lower_message = message.lower()
        if "nodename nor servname provided" in lower_message or "failed to resolve" in lower_message:
            return "dns/network"
        if "timed out" in lower_message or "timeout" in lower_message:
            return "dns/network"
        if "403" in lower_message or "404" in lower_message or "429" in lower_message or "500" in lower_message:
            return "http-status"
